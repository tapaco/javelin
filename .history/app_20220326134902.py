...
if submit: 
    user = auth.create_user_with_email_and_password(email, town)
    db.child(user[email]).child("Name").set(name)
    db.child(user[email]).child("Email").set(email)
    db.child(user[email]).child("Town").set(town)
    db.child(user[email]).child("Single rooms").set(s_rooms)
    db.child(user[email]).child("Double rooms").set(d_rooms)
    db.child(user[email]).child("Room Type").set(room_type)
    db.child(user[email]).child("Women").set(women)
    db.child(user[email]).child("Women & Children").set(women_child)
    db.child(user[email]).child("Photos").set(photos)
    db.child(user[email]).child("Other").set(other)

    st.success('Thank you, your form has been submitted successfully!')
    st.success('A member of our team will be in touch once there is a match')
